...
class PostgressController:
    def get_airports(self, searchTerm: str):
        if str != '':
            ...
            s = 'SELECT "AirportCode", "AirportName","City","Country" FROM "TravelDesk"."AirportDetails" WHERE LOWER("Country") = 'US' ORDER BY "AirportCode" ASC'
        try:
    ...
    def clientAuthentication(self, username: str, password: str):
        ...
        if queryResponse is None:
            ...
            try:
                payload = {
                    'exp': datetime.datetime.utcnow() + datetime.timedelta(days=50, seconds=5),
